...
wxBitmap Wireframe::ApplyShading(int pixelSize){
    ...
    for (Mesh *currMesh : scene->getMeshes()){
        ...
        for (Edge *currEdge : currMesh->getEdges()) {
            vec2 v1_coords = this->scene->getActiveCamera()->projectPoint(currEdge->vertStart->getPos(), this->screenDim);
            vec2 v2_coords = this->scene->getActiveCamera()->projectPoint(currEdge->vertEnd->getPos(), this->screenDim);
            memDC.DrawLine((int)v1_coords[0], (int)v1_coords[1],(int)v2_coords[0], (int)v2_coords[1]);
        }
        for (Vertex *currVert : currMesh->getVerts()){
            vec2 screenCoord = scene->getActiveCamera()->projectPoint(currVert->getPos(), this->screenDim);
