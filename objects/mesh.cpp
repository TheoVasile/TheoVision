...
void Mesh::scale(float size){
    // Scale the vertices around the origin by all axes equally
...
void Mesh::rotate(array<float, 3> rot){
    // Rotate all the vertices around the origin by the specified axes
...
vector<vector<int> > Mesh::getFaces(){
    return this->faces;
}