...
vec2 Camera::projectPoint(vec3 pos, int screenDim[2]) {
    ...
    int scale_factor = std::min(screenDim[1], screenDim[0]);
    vec2 screenCoord(0, 0);
    screenCoord[0] = x_angle / this->fov * 2 * scale_factor + screenDim[0] / 2;
    screenCoord[1] = y_angle / this->fov * 2 * scale_factor + screenDim[1] / 2;
    return screenCoord;
}

vec2 Camera::projectPoint(array<float, 3> pos, int screenDim[2]){
    return this->projectPoint(pos[0], pos[1], pos[2], screenDim);
}

vec2 Camera::projectPoint(float x, float y, float z, int screenDim[2]){
    return this->projectPoint(vec3(x, y, z), screenDim);
}

Ray *Camera::castRay(int x, int y, int screenDim[2])
{
    float angle = this->fov * M_PI / 360;
    float _x = screenDim[0] / 2 - x;
    vec3 xComponent = cross(this->normal, this->vertical) * _x;

    float _y = y - screenDim[1] / 2;
